...
public final class ShopManager {
    ...
    public void load() {
        ...
        if (section == null) {
            plugin.getLogger().warning("No shop categories found.");
            return;
        }
        ...
        for (String id : section.getKeys(false)) {
            ...
            String iconName = config.getString(
                    path + ".icon",
                    "CHEST"
            );
            ...
            if (items != null) {
                for (String itemId : items.getKeys(false)) {
                    ...
                    double buyPrice = config.getDouble(
                            itemPath + ".buy",
                            0.0
                    );

                    double sellPrice = config.getDouble(
                            itemPath + ".sell",
                            0.0
                    );
        ...
        plugin.getLogger().info(
                "Loaded " + categories.size() + " shop categories."
        );
    }
    ...
    public boolean sell(Player player, ShopItem item, int amount) {
        if (amount <= 0 || item.sellPrice() <= 0) {
            return false;
        }
